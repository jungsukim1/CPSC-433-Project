...
def partConstr(assignment, slot, fact):
    tempFact = fact
    for time_slot in tempFact[0]:
        #this if works if jungsu is sending the reference of the actual slot
        if(time_slot is slot):
            print("partial add")
            time_slot.add(assignment)
